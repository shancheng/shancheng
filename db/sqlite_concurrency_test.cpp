 ...
 * 1. Create a SQLite connection in the main thread and use it to another thread
 *    See http://sqlite.org/cvstrac/wiki?p=MultiThreading
 ...
 * 2. SQLite functions get SQLITE_BUSY in case of high concurrency
 * 3. Busy count decreases with the increase of busy timeout configuration value
 */
...
int main(int argc, char* argv[]) {
  ...
  for (size_t i = 0; i < option.threadcount; ++i) {
    ...
    if (sqlite3_busy_timeout(dbvec[i], option.busytimeout) != SQLITE_OK) {
      cerr << "Failed to set busy timeout value (" << sqlite3_errmsg(dbvec[i]) << ")\n";
      goto cleanup;
    }
  }
  for (size_t i = 0; i < option.threadcount; ++i) {
